...
| 8 | 2026-06-27 | Opus 4.8 (1M) Hauptschleife; Subagenten **haiku** (Transkription) / **sonnet** (Task-Reviews + Wiring) / **opus** (Final-Whole-Branch-Reviews) | ~800k (Schätzung; `/context` zuletzt 404k bei 40 % während AP-45; SDD hält die Hauptschleife schlank — Subagent-Arbeit zählt separat) | 29 eigene (2 FF-Merges AP-31/AP-34, 0 Merge-Commits) | 200 → 227 | 0.31.0 → 0.34.0 | ~22 (AP-31 ~14: 6 Impl + 6 Task-Review + 1 Fix + 1 Final/opus; AP-34 ~8: 3 Impl + 3 Task-Review + 1 Fix + 1 Final/opus) | 11 feat / 2 fix (+ 16 docs) | 799 ins. / 31 del Code (16 Dateien; Gesamt-Diff inkl. Doku/Site/Wheels größer) | Playwright (AP-45 12 Checks, 7 Screenshots, Kennzahlen-HTML) + Controller-E2E (launcher core / echtes app.py, AP-31 2-Instanzen-Port-Fallback, run.sh --tray) ✓ | Session 8 (Linux): **AP-45** fertig + v0.32.1; **AP-31-Kern** (Multi-User Pro-Nutzer-Pfade + dynamischer Port, `core/userpaths.py`) v0.33.0 und **AP-34-Kern** (Tray-Icon-Launcher `launcher/`, pystray/Pillow, Ein-Klick via `run.ps1 -Action tray`) v0.34.0 — **beide via Subagent-Driven Development**. **Final-opus-Reviews fingen Integrationslücken außerhalb des Task-Diffs** (AP-31: Launcher-Abbruch bei belegtem Port → gefixt). Daneben: AP-Board lesbarer (5×5-Gitter + zweizeilige Kacheln), Oberfläche-Screenshots neu (komplexer 6-Hop-Pfad), Betriebsseite „Terminalserver", `docs/projekt-kennzahlen.html` (98 % Umsetzung, 89 % Coverage). 8 Sessions an nur 3 Kalendertagen → v0.1.0 → v0.34.0. |
