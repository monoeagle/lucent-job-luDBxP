...
- **AP-67·Oracle-Adaption** — Server-Demo-CMDB für Oracle: **erledigt v0.64.0.** Der `oracle`-Zweig von `sample_data/seed_server_demo.py` baut dieselbe 5-Tabellen-CMDB wie MSSQL + alle Oracle-Objekte (SEQUENCE, MATERIALIZED VIEW, PACKAGE Spec+Body, Procedure/Function, SYNONYM, PL/SQL-TRIGGER, VIEW die Function aufruft → AP-66·S1). `VMCluster` statt `CLUSTER` (reserviert); skip-guardeter Live-Test `tests/test_oracle_seed_integration.py`. **Live gegen Oracle 21c XE verifiziert.** (Folge-Scheibe: Offline-Fixture-Vorschau ohne DB.)
