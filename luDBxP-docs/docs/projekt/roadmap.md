...
- **AP-65** — SQL-Analyzer: **Zeilennummern + Fehler-Lokalisierung** (Konzept: [Analyzer-Zeilen & Fehlerstelle](../../../docs/concepts/2026-06-29-analyzer-line-numbers-error-location.md)). **Stufe A erledigt v0.58.0:** Parse-Fehler zeigen jetzt Zeile/Spalte + markierten Kontext-Ausschnitt (`.an-err-mark`); `AnalysisResult` hat vier neue Felder `parse_error_line/col/context/highlight`; `/api/analyze` serialisiert sie. **Stufe A-Härtung erledigt v0.59.0:** Nicht geschlossenes/verschobenes Anführungszeichen (TokenError) erhält jetzt ebenfalls eine Position + ehrlichen Hinweis; zwei neue `AnalysisResult`-Felder `parse_error_highlight_pos` (kontextrelativer Index, ersetzt `indexOf`) und `parse_error_hint` (Hinweis-Text bei annähernder Position). **Stufe A-Härtung 2 erledigt v0.61.0:** Bei nicht geschlossenem Anführungszeichen zeigt der Analyzer jetzt die **echte Fehlerzeile** statt der am Eingabe-Ende offenen Zeile — der neue reine Helfer `_odd_quote_line(sql, quote_char)` findet die einzige Zeile mit **ungerader** Quote-Anzahl und `_parse_error_location` leitet dorthin um (ohne Spalte/Markierung, da ein fehlendes Quote keine exakte Position hat; deutscher Hinweis nennt die Zeile). Bei mehrdeutiger Zählung bleibt das Eingabe-Ende-Verhalten. **Stufe B erledigt v0.62.0:** Zeilennummern-Gutter im Eingabefeld + Fehlerzeile farbig hinterlegt — reiner Frontend-3-Schicht-Editor (`attachLineGutter`: Gutter · scroll-synchrone Backdrop-Ebene · transparente Textarea, `wrap="off"`), `renderAnalyzeResult` ruft `setErrorLine(parse_error_line)`; NO-CDN, kein Backend-Change, Eingabefeld bleibt verstellbar. **Stufe C erledigt v0.63.0:** Lints/Warnungen mit Zeilenbezug — jede knoten-spezifische Meldung nennt „Zeile N:" (Helfer `_node_line` aus sqlglot-`meta['start']`; neues Feld `line` an `AnalysisWarning`/`AnalysisSuggestion`) und ist anklickbar → markiert die Zeile via AP-65·B-Gutter; Statement-Ebene bleibt ohne Zeile.
...
- **AP-67·Oracle-Adaption** — Server-Demo-CMDB für Oracle: **erledigt v0.64.0.** Der `oracle`-Zweig von `sample_data/seed_server_demo.py` baut dieselbe 5-Tabellen-CMDB wie MSSQL + alle Oracle-Objekte (SEQUENCE, MATERIALIZED VIEW, PACKAGE Spec+Body, Procedure/Function, SYNONYM, PL/SQL-TRIGGER, VIEW die Function aufruft → AP-66·S1). `VMCluster` statt `CLUSTER` (reserviert); skip-guardeter Live-Test `tests/test_oracle_seed_integration.py`. **Live gegen Oracle 21c XE verifiziert.** **Slice 2a erledigt v0.65.0:** reiches Oracle-Schema (~37 Tabellen · 10 Views · 16 Routinen/Packages · Sequences/Synonyme/Matviews, reicher FK-Graph) für komplexe Erkundung. (Die einst geplante Folge-Scheibe 2b „Offline-Fixture-Vorschau ohne DB" wurde am 2026-07-01 **verworfen** — off-mission: das Tool verbindet sich mit echten DBs und untersucht sie, kein Schema-Snapshotting.)
...
- **AP-67·Oracle-Adaption** — Oracle-Server-Demo-CMDB: der `oracle`-Zweig von `sample_data/seed_server_demo.py` (bisher Stub) baut dieselbe 5-Tabellen-CMDB wie MSSQL + alle Oracle-Objektkategorien (SEQUENCE, MATERIALIZED VIEW, PACKAGE Spec+Body, standalone FUNCTION/PROCEDURE, SYNONYM, PL/SQL-TRIGGER, VIEW→Function für AP-66·S1). Oracle-Besonderheiten: per-Objekt-Drop via `BEGIN EXECUTE IMMEDIATE …; EXCEPTION WHEN OTHERS THEN NULL; END;`, `CREATE OR REPLACE`, einzeilige INSERTs, `VMCluster` statt reserviertem `CLUSTER`. Neuer skip-guardeter Live-Test `tests/test_oracle_seed_integration.py` (`LUCENT_ORACLE_TEST_URL`), Bring-up-README (podman Oracle-XE). **Live gegen Oracle 21c XE verifiziert.** **Aufwand M** — v0.64.0
- **AP-67 Slice 2a** — reiches Oracle-Demo-Schema: die Oracle-Demo von 5 auf **~37 Tabellen** (5 Subdomänen) · **10 Views** · **16 Routinen** (8 Functions/4 Procedures/4 Packages) · 3 Sequences · 2 Materialized Views · 3 Synonyme ausgebaut. Reicher FK-Graph (Diamanten, Composite-FK `VMPlacement`, 2 Self-Refs `Folder`/`VMSnapshot`, alternative Routen `Replication`, No-Path `AuditLog`), 6 View→Function-Links (AP-66·S1). DDL im neuen Modul `sample_data/oracle_demo.py` (`CREATE`-dann-`ALTER … ADD FK`-Muster, idempotent); reicher skip-guardeter Integrationstest. **Live gegen Oracle 21c XE verifiziert.** **Aufwand M** — v0.65.0
...
- **AP-65·A-Härtung 2** — Unclosed-Quote → echte Fehlerzeile (Ungerade-Heuristik): neuer reiner Helfer `_odd_quote_line(sql, quote_char)` in `core/sqlanalyze.py` findet die einzige Zeile mit ungerader Anzahl des offenen Quote-Zeichens (die tatsächliche Fehlerzeile). `_parse_error_location` leitet die gemeldete Position im TokenError-Zweig dorthin um, sobald diese Zeile von der am Eingabe-Ende offenen Zeile abweicht — ohne Spalte/Markierung (ein fehlendes Quote hat keine exakte Position), mit deutschem Hinweis, der die Zeile nennt. Bei mehrdeutiger Zählung (0 oder ≥2 ungerade Zeilen, z. B. legitimer mehrzeiliger String) oder wenn die Ungerade-Zeile mit der Eingabe-Ende-Zeile zusammenfällt, bleibt das bisherige Verhalten. Die UI lässt „, Spalte N" weg, wenn keine Spalte vorliegt. Vollständig CI-testbar, kein neues Core-Modul. **Aufwand S** — v0.61.0
- **AP-65·B** — Analyzer Zeilennummern-Gutter + Fehlerzeilen-Highlight: reiner Frontend-3-Schicht-Editor `attachLineGutter(textarea)` in `web/static/js/app.js` wickelt die `#an_sql`-Textarea in Gutter (Zeilennummern, scroll-synchron) · Backdrop-Ebene (eine Zeile je logischer Textzeile; Fehlerzeile trägt `.an-line-error`) · transparente Textarea (`wrap="off"`, bleibt Wert-Träger). `openAnalyzer` verdrahtet `panel._gutter`; `renderAnalyzeResult` ruft `setErrorLine(parse_error_line)` (gelöscht bei Erfolg/Edit). Kein Backend-Change (`parse_error_line` existiert seit Stufe A), NO-CDN (keine Editor-Lib), read-only; Eingabefeld bleibt vertikal verstellbar. **Aufwand S–M** — v0.62.0
- **AP-65·C** — Analyzer Lints mit Zeilenbezug: neuer reiner Helfer `_node_line(node, sql)` in `core/sqlanalyze.py` leitet die 1-basierte Zeile aus dem frühesten positionierten sqlglot-Nachfahren ab (`meta['start']`); `AnalysisWarning`/`AnalysisSuggestion` erhalten ein Feld `line: int | None`. Die knoten-spezifischen Meldungen (SELECT *, LIKE '%…', Funktion-auf-Spalte, verdächtiger Alias, kartesischer Join, unbekannte Tabelle/Spalte, OR in WHERE, Unterabfrage in WHERE) tragen die Zeile; Statement-Ebene (WRITE_STATEMENT/NO_WHERE/…) bleibt `None`. `/api/analyze` serialisiert `line`; die UI präfixt „Zeile N:" und macht die Meldung anklickbar → markiert die Zeile via AP-65·B-Gutter (`setErrorLine`, delegierter `#an_result`-Listener). Read-only, NO-CDN, kein neues Core-Modul. **Aufwand M** — v0.63.0
